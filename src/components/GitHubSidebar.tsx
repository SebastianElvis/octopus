...
const CI_STATUS_PILL: Record<string, string> = {
  ...
  failure: "bg-red-500/20 text-red-400 ring-1 ring-red-500/30",
  pending: "bg-yellow-500/20 text-yellow-400 ring-1 ring-yellow-500/30",
  unknown: "bg-gray-500/20 text-gray-400 ring-1 ring-gray-500/30",
};
...
export function GitHubSidebar({
  ...
  return (
    <div className="flex flex-col gap-4">
      <h3 className="text-sm font-medium uppercase tracking-wide text-gray-400">GitHub</h3>
        ...
        <>
          ...
          {issueError && !loadingIssue && (
            <div className="rounded-lg border border-red-800/40 bg-red-950/20 p-3">
              <p className="text-xs text-red-400">Failed to load issue: {issueError}</p>
            </div>
          )}
          {issue && !loadingIssue && (
            <div className="rounded-lg border border-gray-800 bg-gray-900 p-3">
              ...
              <p className="text-sm font-medium text-gray-100">{issue.title}</p>
              {issue.labels.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-1">
                  {issue.labels.map((label) => (
                    <span
                      key={label}
                      className="rounded-full bg-blue-500/20 px-2 py-0.5 text-xs text-blue-400 ring-1 ring-blue-500/30"
                    >
                      {label}
                    </span>
                  ))}
      ...
      {linkedPRNumber && (
        <>
          ...
          {prError && !loadingPR && (
            <div className="rounded-lg border border-red-800/40 bg-red-950/20 p-3">
              <p className="text-xs text-red-400">Failed to load PR: {prError}</p>
            </div>
          )}
          {pr && !loadingPR && (
            <div className="rounded-lg border border-gray-800 bg-gray-900 p-3">
              ...
              <p className="text-sm font-medium text-gray-100">{pr.title}</p>
      ...
      {!pr && hasCommittedChanges && repoId && branch && (
        <div>
          {createError && <p className="mb-1 text-xs text-red-400">{createError}</p>}
          <button
            onClick={() => {
              void handleOpenPR();
            }}
      ...
      {!linkedIssueNumber && !linkedPRNumber && !hasCommittedChanges && (
        <p className="text-xs text-gray-600">No GitHub context linked.</p>
      )}
    </div>
  );
}
