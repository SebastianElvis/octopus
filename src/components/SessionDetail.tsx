  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center gap-2 text-sm">
        ...
        <span className="text-gray-100">{session.name}</span>
      </div>
...
function SessionHeader({ session, onInterrupt }: { session: Session; onInterrupt: () => void }) {
  return (
    <div className="flex items-start justify-between rounded-lg border border-gray-800 bg-gray-900 px-5 py-4">
      <div className="flex flex-col gap-1">
        ...
        <div className="flex items-center gap-3 text-sm text-gray-500">
          ...
          {session.branch && (
            <>
              <span className="text-gray-700">·</span>
              <span className="font-mono text-xs text-gray-400">{session.branch}</span>
            </>
          )}
          <span className="text-gray-700">·</span>
...
function BlockerBanner({
  ...
  return (
    <div className="rounded-lg border border-red-800/60 bg-red-950/30 p-4">
      ...
      {session.lastMessage && (
        <div className="mb-3 rounded-md bg-gray-900/60 p-3 font-mono text-sm text-gray-300">
          {session.lastMessage}
        </div>
      )}
      ...
      <div className="flex gap-2">
        <textarea
          ...
          className="flex-1 resize-none rounded-md border border-gray-700 bg-gray-900 px-3 py-2 text-sm text-gray-100 placeholder-gray-600 focus:border-blue-600 focus:outline-none"
        />
        <button
          onClick={() => {
            onSend();
      ...
      <p className="mt-1 text-xs text-gray-600">Cmd+Enter to send</p>
    </div>
  );
}
