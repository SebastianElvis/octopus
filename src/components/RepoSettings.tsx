  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <h2 className="text-base font-semibold text-gray-100">Repositories</h2>
        <button
      ...
      {showForm && (
        <div className="rounded-lg border border-gray-800 bg-gray-900 p-4">
          <h3 className="mb-3 text-sm font-medium text-gray-300">Add Repository</h3>
          <div className="space-y-3">
            ...
            <div>
              <label className="mb-1 block text-xs font-medium text-gray-400">Local Path</label>
              <input
                type="text"
                value={localPath}
          ...
          <div className="mt-3 flex justify-end gap-2">
            <button
              ...
              className="rounded-md border border-gray-700 px-3 py-1.5 text-sm text-gray-300 hover:border-gray-600"
            >
              Cancel
            </button>
            <button
      ...
      {loadingRepos ? (
        <div className="space-y-2">
          {[1, 2].map((n) => (
            <div
              key={n}
              className="flex animate-pulse items-center justify-between rounded-lg border border-gray-800 bg-gray-900 px-4 py-3"
            >
              <div className="space-y-2">
          ...
          {repos.map((repo) => (
            ...
            >
              <div className="min-w-0">
                <p className="truncate text-sm font-medium text-gray-100">{repo.githubUrl}</p>
                <p className="truncate text-xs text-gray-500">{repo.localPath}</p>
                <p className="mt-0.5 text-xs text-gray-600">Default branch: {repo.defaultBranch}</p>
              </div>
              <button
                onClick={() => removeRepo(repo.id)}
                className="ml-3 shrink-0 rounded-md border border-gray-700 px-2.5 py-1 text-xs text-red-400 hover:border-red-800 hover:text-red-300"
              >
                Remove
              </button>
            </div>
